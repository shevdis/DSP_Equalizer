...
public class FXMLDocumentController implements Initializable {
    ...
    private void clippingBox() {
        ...
        if (!this.audioPlayer.clippingIsActive())
            this.audioPlayer.setClipping(true);
        else this.audioPlayer.setClipping(false);
    }
    ...
    private void listenSliders() {
        ...
        Slider9.valueProperty().addListener((observable, oldValue, newValue) -> {
            String str = String.format("%.3f", (newValue.doubleValue()));
            Label9.setText(str);
            audioPlayer.getEqualizer().getFilter(9).setGain(newValue.doubleValue());
        });
    }
}
